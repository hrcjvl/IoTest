...
if(platform.system()=="Windows"):
    if "64bit" in platform.architecture():
        ...
        dll_path = os.path.join(script_dir, "lib\\windows\\64bit\\Ginkgo_Driver.dll")
        print ("load 64bits dll")
    else:
        # GinkgoLib = windll.LoadLibrary( ".\\lib\\windows\\32bit\\Ginkgo_Driver.dll" )
        dll_path = os.path.join(script_dir, "lib\\windows\\32bit\\Ginkgo_Driver.dll")
        print ("load 32bits dll")
    GinkgoLib = windll.LoadLibrary(dll_path)
elif(platform.system()=="Darwin"):
    GinkgoLib = cdll.LoadLibrary( "./lib/macos/libGinkgo_Driver.dylib" )
elif(platform.system()=="Linux"):
    if "64bit" in platform.architecture():
        dll_path = os.path.join(script_dir, "lib\\linux\\64bit\\libGinkgo_Driver.so")
        # GinkgoLib = cdll.LoadLibrary( "./lib/linux/64bit/libGinkgo_Driver.so" )
    else:
        dll_path = os.path.join(script_dir, "lib\\linux\\32bit\\libGinkgo_Driver.so")
        # GinkgoLib = cdll.LoadLibrary( "./lib/linux/32bit/libGinkgo_Driver.so" )
    GinkgoLib = cdll.LoadLibrary(dll_path)
else:
